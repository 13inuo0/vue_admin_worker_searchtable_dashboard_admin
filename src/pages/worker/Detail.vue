<template>
  <div>
    <div v-if="!job" class="p-4 text-center text-gray-500">작업 정보를 받아오는 중 ...</div>
    ...
    <div v-if="job" class="bg-white border border-gray-200 p-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <button v-if="signature" @click="clearSingnature" class="text-xs text-red-600">삭제</button>
      </div>
      <!-- 서명 캔버스 -->
...
// 라우터 쿼리에서 jobId 가져와 작업 데이터 로드하기
const job = ref(null);
...
function onFilesSelected(e) {
  ...
  // Promise.all()란 모든 Promise객체를 처리하는 메서드
...
function draw(e) {
  ...
  signature.value = canvas.toDataURL(); //캔버스를 data URL형식으로 변환하는 메서드
}
...
function submitWork() {
  ...
  alert("작업이 완료되었습니다. 정산 내역을 확인 할 수 있습니다.");
  // 작업목록으로 넘어가기
