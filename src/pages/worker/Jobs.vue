  ...
  <div class="w-full">
    ...
    <div class="px-4 pt-3">
      <div class="flex gap-2">
        <button
          class="px-3 py-2 rounded-full text-sm border shadow-sm active:scale-[0.98] transition-colors"
        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            <p class="font-semibold text-base flex items-center justify-between text-gray-900 dark:text-white">
              <span>
                {{ job.storeName }}<span class="text-gray-400 dark:text-gray-500"> - {{ job.time }}</span>
              </span>
            </p>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ job.address }}</p>
            <div class="mt-2 flex items-center gap-2">
              <span :class="statusBadge(job.status)" class="text-[11px] px-2 py-1 rounded-full font-medium">
    ...
    <!-- 작업 목록 리스트 비어있을 때 -->
    <div v-if="filteredJobs.length === 0">할당된 청소 작업이 없습니다.</div>
  </div>
</template>
<script setup>
...
function gotoWorkDeatial(job){
// 작업 수행 페이지로 이동(작업 아이디를 쿼리 파라미터로 전달 받기)
router.push({
name:"WorkerDetail",
query:{jobId: job.id},
