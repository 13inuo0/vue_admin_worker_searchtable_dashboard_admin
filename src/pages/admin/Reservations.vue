...
import { resersData } from "@/data/reser";
...
const reserFilterFn = (data, filters) => {
  // filters = select태그의 v-model = 필터옵션의 key.value
  let result = [...data];
  // console.log(result);
  // console.log(filters);
...
window.handleRservationCancel = (id) => {
  const reservation = reservations.value.find(
    // String(id) = id를 문자열로 변환하기
    (r) => r.id === id || r.id === String(id)
  );
...
// 예약상세 모달
const closeModal = () => [(selectedReser.value = null)];
  ...
  if (filters.area && filters.area !== "all") {
    result = result.filter((tech) => {
      ...
      return (
        ...
        // includes() 문자열에 특정 문자열이 표함되어 있는지 확인
...
window.handleSelectTechnician = (id) => {
  ...
  if (technician) {
    selectedTeach(technician);
  }
};

// 기사 선택시 실행되는 함수
const selectedTeach = (technician) => {
